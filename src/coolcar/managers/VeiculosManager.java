...
public class VeiculosManager {
	public ArrayList<Veiculo> consulta(Veiculo veiculo) {
		...
		String placaStr, anoStr, filialStr, statusStr, arCondStr, dirHidStr,
			   cambAutoStr, cilindradasStr, tamTanqueStr;
		String sql;
		...
		try {
		    ...
		    if(veiculo.getTipoDoVeiculo() == "Carro"){
			    ...
			    while (resultados.next()) {			    
				    ...
				    Carro carro_select = new Carro(resultados.getString("nome"),
				    							   ...
				    							   new BigDecimal(resultados.getFloat("diaria")),
					    						   resultados.getInt("id_modelo"),
			    	...
			    	Veiculo veiculo_select = new Veiculo("Carro",
			    			 							...
			    			 							resultados.getInt("kilometragem"),
			    			 							resultados.getInt("ano"),
			    			 							resultados.getString("status"),
			    			 							resultados.getInt("id_funcionario_veic"),
			    			 							resultados.getInt("id_modelo"),
		    ...
		    /*----------------------------------------------------------------------------------------MOTO-*/
			    ...
			    sql = "SELECT * FROM veiculo NATURAL JOIN (modelo NATURAL JOIN moto)"
		        		+ "WHERE placa = " + placaStr + " AND ano = " + anoStr + " AND filial_alojada = " + filialStr + " AND"
		        		+ " status = " + statusStr + " AND cilindradas = " + cilindradasStr + " AND tamanho_tanque = " + tamTanqueStr; 
			    PreparedStatement stmt = connection.prepareStatement(sql);
	        	
			    ResultSet resultados = stmt.executeQuery();
			    while (resultados.next()) {
				    Moto moto_select = new Moto(resultados.getString("nome"),
												resultados.getString("fabricante"),
												resultados.getString("combustivel"),
												resultados.getString("tipo"),
												new BigDecimal(resultados.getFloat("diaria")),
